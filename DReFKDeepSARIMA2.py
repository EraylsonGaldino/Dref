...
import time
''' Esse arquivo contém CNN e LSTM  como modelos deep '''
class Dynamic:
    ...
    def start(self):
       ...
       #  Ctrainarima = ts(numeric(arima_train),frequency=self.dimension) SARIMA
       # trainarima = ts(numeric(arima_train)) PRA ARIMA
       
        trainarima = ts(np.array(arima_train),frequency=self.dimension) 
        inicio = time.time()
        fit = ets((trainarima))
        fim = time.time()
        time_train_ARIMA = fim-inicio
        ordemModelo = arimaorder(fit)
       # print('SARIMA(%d, %d, %d)(%d, %d, %d)_%d'%(ordemModelo[0],ordemModelo[1],ordemModelo[2],ordemModelo[3],ordemModelo[4],ordemModelo[5],ordemModelo[6]))
        fitted = r.r('fitted')
        predTreino = fitted(fit)
        residTreino=np.asarray(arima_train)-np.asarray(predTreino)
        ArimaTest = r.r('Arima')
        fitval= ArimaTest(numeric(arima_val),  model=fit)
        predVal=np.asarray(fitted(fitval))
        residVal=np.asarray(arima_val)-np.asarray(predVal)
        fitvalf2= ArimaTest(numeric(arima_valf),  model=fit)
        predValf2=np.asarray(fitted(fitvalf2)) 
        residVal2=np.asarray(arima_valf)-np.asarray(predValf2)
        inicio = time.time()
        fittest = ArimaTest(numeric(arima_test), model=fit)
        fim = time.time()
        ...
        for i in range(nlgs+1):
            if (confs[i][0]>acs[i] or acs[i]>confs[i][1]):
                newlags.append(i)
        indexMax = np.argmax(acs[1:])
        indexMin = np.argmin(acs[1:])
        if(np.abs(acs[indexMax+1])>np.abs(acs[indexMin+1])):
            lags = indexMax
        else:
            lags = indexMin
        ...
        #rna = mgs.myfit(nn1,train_set2, train_target2,val_set2,val_target2,param_grid )
        ...
        #svr.fit(train_set2, train_target2,val_set2,val_target2,scoring='neg_mean_squared_error')
        ...
        #n_hidden
        rbf=RBFNet(k=3) # TROCAR PARA rbf !!!!!
        
        #rbf = GridSearchCV(rbf1, param_grid={
        #    'k': [3, 5, 10, 15]})

        rbf.fit(np.asarray(train_set2), np.asarray(train_target2)) 
        #param_grid={  'k': [3, 5, 10, 15]}
        #rbf =mgs.myfit(rbfm,np.asarray(train_set2), np.asarray(train_target2),np.asarray(val_set2), np.asarray(val_target2),param_grid)
        #rbf.fit(train_set2, train_target2,val_set2,val_target2,scoring='neg_mean_squared_error')
...
#        predArimaTest = np.asarray(fitted(ArimaTest))
 ...
 #       (predDBNtreino, predDBNval1, predDBNval2, predDBNtest)= self.readXLXs(self.fileDBN)
    ...
    #            mseDBNRES = mean_squared_error((val_target2[knearest])[conts], predDBNResVal)
        ...
        writer = pd.ExcelWriter('%s_2_SARIMAV2_EXTRA.xlsx'%(self.dataname), engine='xlsxwriter')
        ...
        return (min(mseEU['Values'][6],mseDTW['Values'][6]),K)
